// use std::fs;
// use std::io::{self, Write}; // Import io and Write trait for flush
...
//     let path = "hidden_words.json"; // Ensure this path is correct relative to your executable or run directory
//     println!("Attempting to read JSON file from: {}", path);
//     let json_content = fs::read_to_string(path)?;

//     // 2. Parse the JSON content
//     let data: Root = serde_json::from_str(&json_content)?;
//     println!("JSON file parsed successfully.");

//     loop {
//         // 3. Get user input for list choice
//         println!("\nChoose a word list:");
...
//         let choice = choice.trim(); // Remove leading/trailing whitespace and newline
...
//         // 4. Select the appropriate word list based on user input
//         let selected_words: Option<&Vec<String>> = match choice {
//             "1" => Some(&data.word_lists.four_letter_words),
//             "2" => Some(&data.word_lists.five_letter_words),
//             "3" => Some(&data.word_lists.six_letter_words),
//             _ => {
...
//             if words.is_empty() {
//                 println!("The selected list is empty. Please check your JSON file.");
//                 continue;
//             }

//             // 5. Randomly select a word from the chosen list
//             let mut rng = thread_rng(); // Initialize a random number generator
//             let random_word = words.choose(&mut rng).expect("Should be able to pick a word from a non-empty list");

//             println!("Selected word: {}", random_word);
//         }

//         // Add a pause or prompt to continue if needed
//         println!("\nPress Enter to continue...");
//         io::stdout().flush()?; // Ensure the prompt is displayed
//         let mut _temp = String::new(); // Read an empty line to pause
